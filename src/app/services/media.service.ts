...
import {
  ...
  SelectedImageConfig,
} from '../dashboard/media-library/media-library.component';
...
export class MediaService {
  ...
  public async deleteImage(
    id: number,
  ): Promise<ApiResponse<MediaDeletionResult>> {
    try {
      return firstValueFrom(
        this.http.delete<ApiResponse<MediaDeletionResult>>(`api/media/${id}`),
      );
    } catch (e) {
      return {
        success: false,
  ...
  public async editImage(
    id: number,
    edit: ImageEditEvent,
  ): Promise<ApiResponse<MediaInterface>> {
    try {
      return firstValueFrom(
        this.http.patch<ApiResponse<MediaInterface>>(`api/media/${id}`, edit),
      );
    } catch (e) {
      return {
        success: false,
  ...
  public async getValidFileName(fileName: string): Promise<string> {
    try {
      const newNameResult = await firstValueFrom(
        this.http.get<ApiResponse<{ validName: string }>>(
  ...
  public async loadMedia(page: number): Promise<void> {
    const result = await firstValueFrom(
      this.http.get<PaginatedApiResponse<Image>>(`api/media/${page}`),
    );
    if (result.success) {
  ...
  public async upload(
  ...
  ): Promise<ApiResponse<MediaCreationResult>> {
    ...
    try {
      ...
      const result = await firstValueFrom(
        this.http.post<ApiResponse<MediaCreationResult>>(
          'api/media/create',
          fd,
        ),
      );
